...
export function TaxRequestsClient() {
  const { data, mutate } = useSWR<{ requests: RequestRow[] }>("/api/admin/tax-requests", fetcher);
  ...
  async function review(request: RequestRow, status: "approved" | "rejected") {
    ...
    const res = await fetch("/api/admin/tax-requests", {
      method: "PATCH",
  ...
  const columns: ColumnDef<RequestRow>[] = [
    ...
    {
      id: "details",
      header: "Requested values",
      cell: ({ row }) => (
        <div className="grid grid-cols-2 gap-3">
          <Field label="TIN" value={row.original.tin} />
          <Field label="Tax office" value={row.original.taxOffice} />
          <Field label="PFA" value={row.original.pfaName} />
          <Field label="RSA PIN" value={row.original.rsaPin} />
        </div>
      ),
    },
    {
  ...
  return (
    <div>
      <PageHeader
        title="Tax & Pension Change Requests"
        description="Review employee requests to update TIN, tax office, PFA or RSA PIN. Approving applies the new values to the employee record."
      />

      {requests.length === 0 ? (
        <EmptyState
          title="No tax/pension requests"
          description="Requests submitted by employees through My Account will appear here."
        />
      ) : (
        <Card className="overflow-hidden p-2">
          <DataTable
            columns={columns}
            data={requests}
            filterKeys={["employee.firstName", "employee.lastName", "status", "tin"]}

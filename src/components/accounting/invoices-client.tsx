...
import { InvoiceDetail, type InvoiceRow } from "@/components/accounting/invoice-detail";
...
export function InvoicesClient({ permissions }: { permissions?: Set<string> }) {
  ...
  const paidAmount = (inv: InvoiceRow) =>
        ...
        <div className="flex items-center justify-end gap-1">
          ...
          {row.original.status !== "paid" && row.original.status !== "cancelled" && (
            <Button
              ...
              className="h-7"
              onClick={() => {
                setPayInvoice(row.original as unknown as Record<string, any>);
                setPayOpen(true);
              }}
            >
      ...
      {invoices.length === 0 ? (
        ...
        <Card className="overflow-hidden p-2">
          <DataTable
            ...
            filterKeys={["invoiceNumber", "customer.name", "status"]}
